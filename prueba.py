...
5.1 Mostrar texto de ganar o de perder (hecho)
...
class Cuerpo():
    ...
    def imprimirenventana(self):
        for filas in range(0,9):
            for columnas in range(0,9):
                boton = tkinter.Button(ventana,text=self.tablero[filas][columnas],height=2,width=4,command=self.menunumeros)
                boton.grid(row=filas,column=columnas)
                
                
                #MODIFICAR TABLERO
                
                if self.tablero[filas][columnas] != 0:
                    boton.config(fg="red",bg="#ffb3b3")

                boton_modificar = tkinter.Button(ventana,text="Colocar",height=1,width=2)
                boton_modificar.grid(row=1,column=10)

    def menunumeros(self):
        caja = tkinter.Entry(ventana)
        caja.grid(row=0,column=11)
...
s_cuerpo.imprimirenventana()

#s_cuerpo.menunumeros()
